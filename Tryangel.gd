extends Area2D
const LoadingMainMenuInBackground = preload("res://GameOver.tscn")
...
"""	if IsMoving == true: 
	...
	if istInDerWischkette == true and tryangle2.istInDerWischkette == true and tryangle.istInDerWischkette == true and tryangle3.istInDerWischkette == true and ControlledPoint == tryangle2.ControlledPoint and ControlledPoint > tryangle.ControlledPoint and tryangle.ControlledPoint > tryangle3.ControlledPoint:
			...
			line_2d.set_point_position(line_2d.all_points-4,tryangle3.position)"""
func _process(delta):
	if istInDerWischkette and not verkleinert:
		scale.x = 0.3
		scale.y = 0.31
	elif not istInDerWischkette and not verkleinert:
		scale.x = 0.25
		scale.y = 0.258
	if IsMoving: 
		position.y += GlobalSettings.GlobalSpeed() * delta  
		rotation_degrees += 25 * delta 
	if tryangle.istInDerWischkette and tryangle2.istInDerWischkette and tryangle3.istInDerWischkette and not verkleinert:
		verkleinert = true
...
func _on_area_2d_area_entered(area):
	if line_2d:
		if line_2d.isCube == false and line_2d.isHex == false and not position.y == 3000 and not verkleinert:
			...
			SelectAnim()
			line_2d.default_color = Color(1, 0.071, 0.98, 1)
			line_2d.isTryangle = true
			line_2d.isCube = false
			line_2d.isHex = false
			ControlledPoint = line_2d.all_points
		elif line_2d.isCube == true and not position.y == 3000:
			get_tree().change_scene_to_file("res://GameOver.tscn")
		elif line_2d.isHex == true and not position.y == 3000:
			get_tree().change_scene_to_file("res://GameOver.tscn")
func SelectAnim():
