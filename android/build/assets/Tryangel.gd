...
func _process(delta):
	...
	if IsMoving: 
		position.y = position.y + 1
		rotation_degrees = rotation_degrees + 0.2
	if tryangle.istInDerWischkette and tryangle2.istInDerWischkette and tryangle3.istInDerWischkette:
...
func _on_area_2d_area_entered(area):
	if line_2d:
		if line_2d.isCube == false and line_2d.isHex == false and not position.y == 3000:
